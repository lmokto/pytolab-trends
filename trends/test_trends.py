...
import db
import trends
#from trends.exceptions as exceptions
...
class Test(unittest.TestCase):
    ...
    def test_process_post_names_not_found(self, rpush_mock, set_post_mock,
            incr_mock):
        self.trends.persons = [
            {'name': 'Laura', 'first_name': 'Paonessa',
             'nickname': 'laurapaonessa', 'id': 2, 'words': ()},
            {'name': 'Cristian', 'first_name': 'Reynaga',
             'nickname': '_creynaga', 'id': 2, 'words': ()}]
        self.trends.db = db.Db()
        self.trends.stats_last_update = 0
        post = {'text': 'test'}
